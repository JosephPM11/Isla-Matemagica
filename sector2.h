...
extern void asetColor(int textColor);

// Función para fondo (Para evitar error de que no se coloree todo)
extern void afillConsoleBackground(int backgroundColor);
...
void enemigo_s2(){
	...
	cout<<"\n"<<endl;
	if(*boss_s2==1){
		afillConsoleBackground(BACKGROUND_CYAN);
		asetColor(FOREGROUND_RED | FOREGROUND_INTENSITY);
		cout<<"\n\n\n";   
		...
		cout<<"\t\t\t\t\t\t   __-||| ||||||| |-- __"<<endl;
		cout<<"\t\t\t\t\t\t  |__| |||*||T||*||| |__|"<<endl;
		asetColor(FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE | FOREGROUND_INTENSITY);
	}
	else{
		afillConsoleBackground(BACKGROUND_CYAN);
		asetColor(FOREGROUND_RED | FOREGROUND_BLUE | FOREGROUND_INTENSITY);
		cout<<"\n\n\n";
